...
class Scene {
    ...
    load() {
        return new Promise((resolve, reject) => {
           new FBXLoader().load(this.data.asset, async fbx => {
                ...
                this.cameraProps.target = new THREE.Vector3().fromArray(this.data.target || [0, 0, 0])

                if (this.data.campos) {
                    this.cameraProps.position = new Vector3().fromArray(this.data.campos)
                }

                if (!this.cameraProps.position) {
                    const cam = this.object.getObjectByName('camera1') ||
