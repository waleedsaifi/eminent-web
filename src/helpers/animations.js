...
export const getAnimationTimeout = (currentStep, currentSectionTitle) => {
...
export const removeInlineStyles = (elArr) => {
  ...
  elArr.forEach(
    (el) => el && el.hasAttribute("style") && el.removeAttribute("style")
  );
};

export const stopCustomAnimationSvg = ([svgTextClass, svgTextClass_Path]) => {
  window.animation &&
    window.animation.remove([svgTextClass, svgTextClass_Path]);
...
export const stopFormTenAnimation = (classes) => {
  const [majorClass, minorClass] = classes;
  window.chooseStoryAnimation &&
    window.chooseStoryAnimation.remove([majorClass, minorClass]);
  const elements = classes.reduce((acc, curr) => {
    const partElements = [...document.querySelectorAll(curr)];
    return [...acc, ...partElements];
  }, []);
...
export const getFadeInChooseStoryText = (callback) => {
  window.chooseStoryAnimation = anime.timeline().add({
...
export const getFadeOutFormTen = (targets, delay = 0, callback) => {
  window.formTenAnimation = anime.timeline().add({
    targets,
    opacity: [1, 0],
    easing: "easeOutQuad",
    delay: delay,
...
export const rightMenuBtnHoverInAnimation = (
  menuRightBtn,
  rightBorderBtn,
  startBg
) => {
  window.animation_t1 = anime
    .timeline()
    .add({
...
export const rightMenuBtnHoverOutAnimation = (
  rightBorderBtn,
  menuRightBtn,
  bg,
  callback
